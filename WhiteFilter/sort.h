...
public:
	...
	int min(int a,int b);
	int search(vector<Type> &v,Type key,int lo,int hi);//二分查找，递归法。找不到返回-1
	int search(vector<Type> &v,Type key);//二分查找，非递归法。找不到返回-1
	//思考：二分法查找若能找到则返回该元素的位置，即小于该元素的个数，若找不到，则还是返回表中小于它的元素的个数
};
...
template <class Type>
int sort<Type>::search(vector<Type> &v,Type key,int lo,int hi)
{
	if(hi<lo)
		return -1; 
	int mid=lo+(hi-lo)/2;
	if(key<v[mid])
		return search(v,key,lo,mid-1);
	else if(key>v[mid])
		return search(v,key,mid+1,hi);
	else
		return mid;
}

template <class Type>
int sort<Type>::search(vector<Type> &v,Type key)
{
	int mid;
	int lo=0;
	int hi=v.size()-1;
